...
def test_MechanicalTestDataPreprocessor_init_default():
    ...
    assert (
        set(unit for sublist in preprocessor.KNOWN_UNITS.values() for unit in sublist) == preprocessor.flat_known_units
    )
...
def test_MechanicalTestDataPreprocessor_init_custom():
    ...
    preprocessor = MechanicalTestDataPreprocessor(
        column_mapping=custom_mapping, expected_units=custom_units, known_units=custom_known_units
    )
