...
class WidgetManager:
    ...
    def create_entry_group(self):
        ...
        self.name_entry, self.length_entry, self.width_entry, self.thickness_entry, self.weight_entry = (
            self.entry_group.entries
        )
    ...
    def create_data_analysis_button_group(self):
        data_analysis_names = ["Submit", "Test Hide", "Plot Average", "Recalculate export KPI", "Clear Specimen"]
        data_analysis_functions = [
            self.button_actions.submit,
            self.create_visibility_controller,
            self.button_actions.plot_average,
            self.button_actions.recalculate_specimen,
            self.button_actions.delete_selected_specimens,
        ]
        data_analysis_specs = list(
            zip(
                data_analysis_names,
                data_analysis_functions,
    ...
    def create_data_management_button_group(self):
        data_management_names = [
            "Import Specimen",
            "Save Specimen",
            "Export Average to Excel",
            "Update Plot",
            "Individual Excel Files",
        ]
        data_management_functions = [
            self.button_actions.import_data,
            self.button_actions.save_selected_specimens,
            self.button_actions.export_average_to_excel,
            self.button_actions.update_plot,
            self.button_actions.export_indivdual_data,
        ]
        data_management_specs = list(
            zip(
                data_management_names,
                data_management_functions,
    ...
    def create_fifth_row_group(self):
        self.fifth_row_group = FifthRowGroup(
            self.app.master,
            reset_callback=self.reset_sliders,
    ...
    def create_notebook(self):
        ...
        self.scrollable_notebook_frame = ScrollableFrame(self.app.master)
        # self.scrollable_notebook_frame.grid(row=7, column=0, columnspan=8, sticky="nsew")

        # Create the notebook inside the scrollable frame
        self.notebook = ttk.Notebook(self.scrollable_notebook_frame.scrollable_frame)
        self.notebook.grid(row=0, column=0, sticky="nsew")
        self.notebook.bind("<<NotebookTabChanged>>", self.on_tab_change)

        # Configure the grid inside the scrollable_frame's scrollable_frame to allow the notebook to expand
        self.scrollable_notebook_frame.scrollable_frame.grid_rowconfigure(0, weight=1)
        self.scrollable_notebook_frame.scrollable_frame.grid_columnconfigure(0, weight=1)
    ...
    def configure_layout(self):
        # Place each frame in the master grid with centralized layout configuration
        self.app.master.grid_columnconfigure((0, 1, 2, 3, 4, 5, 6, 7), weight=1)
        self.app.master.grid_rowconfigure(7, weight=1)

        # Position each frame in the master grid layout
    ...
    def update_ui_elements(self, filename, specimen):
        ...
        # use a toast notification to inform the user that the data has been imported
        # from tkinter bootstrap import Toast

        from ttkbootstrap.toast import ToastNotification

        ToastNotification(
            title="Data Import", message=f"Data has been imported successfully from {filename}!", duration=3000
        ).show_toast()
    ...
    def on_tab_change(self, event):
        ...
        # Only proceed with plotting if there is more than one specimen

        if len(self.app.variables.specimens) > 1:
            # Check if the number of specimens is within the threshold for automatic plotting
            if len(self.app.variables.specimens) <= 30:
                self.button_actions.plot_all_specimens()
            else:
                print(
                    "Warning: The number of specimens exceeds 30. `plot_all_specimens` needs to be called manually for large samples to avoid potential memory issues."
                )
    ...
    def toggle_slider(self, *args):
        for slider_manager in [
            self.app.plot_manager.slider_managers["left"],
            self.app.plot_manager.slider_managers["middle"],
        ]:
            if slider_manager is not None:  # Right plot has no slider
                if self.slider_enabled.get():  # if toggle button is checked
                    ...
                    self.app.plot_manager.specimen.graph_manager.strain_shifted = (
                        self.app.plot_manager.specimen.shifted_strain
                    )
    ...
    def get_save_file_path(self):
        ...
        return filedialog.asksaveasfilename(
            title="Save the average curve to an Excel file",
...
class FifthRowGroup(tk.Frame):
    def __init__(
        ...
        enable_strain_callback=None,
        enable_select_callback=None,
        ms_word_callback=None,
        slider_enabled=None,
        select_mode_enabled=None,
        internal_plot_enabled=None,
        external_plot_enabled=None,
        **kwargs,
    ):
        super().__init__(master, **kwargs)
    ...
    def create_strain_checkbox(self, callback):
        self.toggle_button = tk.Checkbutton(
            self, text="Enable strain shift", variable=self.strain_variable, command=callback
        )
        self.toggle_button.grid(row=1, column=0, padx=10, pady=10, sticky="n")

    def create_select_checkbox(self, callback):
        self.select_mode_toggle_button = tk.Checkbutton(
            self, text="Enable Select Mode", variable=self.select_variable, command=callback
        )
    ...
    def create_internal_plot_checkbox(self):
        self.internal_plot_toggle_button = tk.Checkbutton(
            self, text="Internal Plot", variable=self.internal_plot_variable
        )
        self.internal_plot_toggle_button.grid(row=1, column=5, padx=10, pady=4, sticky="n")

    def create_external_plot_checkbox(self):
        self.external_plot_toggle_button = tk.Checkbutton(
            self, text="External Plot", variable=self.external_plot_variable
        )
...
class PrelimGroup(tk.Frame):
    ...
    def create_plateau_stress_entry(self):
        ...
        self.plateau_stress_entry = PlaceholderEntry(
            self, placeholder="Plateau Stress (MPa)", textvar=self.plateau_stress_var
        )
    ...
    def update_calculation_results(self):
        try:
            ...
            self.calculation_results_label.config(
                text=f"Key Stress: 20%: {(plateau_stress * 0.2):.3f} 70%: {plateau_stress * 0.7:.3f} 130%: {plateau_stress * 1.3:.3f}"
            )
        except ValueError:
    ...
    def update_trigger_forces_labels(self):
        if self.plateau_stress_var.get() and self.app.variables.current_specimen:
            ...
            self.trigger_forces_label.config(
                text=f"Trigger Forces (N): {forces[0]:.2f}, {forces[1]:.2f}, {forces[2]:.2f}"
            )
    ...
    def calculate_and_display_average_stress(self):
        if not self.prelim_mode.get():
            if self.app.variables.current_specimen:
                if (self.range_start_var.get() != self.range_entry_start.placeholder) and (
                    self.range_end_var.get() != self.range_entry_end.placeholder
                ):
                    try:
...
class VisibilityControllerWindow:
    ...
    def create_toggle_buttons(self):
        ...
        tk.Checkbutton(
            self.window, text="Toggle Prelim Group", var=self.prelim_group_visible, command=self.toggle_prelim_group
        ).pack()

        # Toggle button for the entry group
        tk.Checkbutton(
            self.window, text="Toggle Entry Group", var=self.entry_group_visible, command=self.toggle_entry_group
        ).pack()

        # Toggle button for the plot title entry group
        tk.Checkbutton(
            self.window,
            text="Toggle Plot Title Entry Group",
            var=self.plot_title_entry_group_visible,
            command=self.toggle_plot_title_entry_group,
        ).pack()

        # Toggle button for the ticker widgets
        tk.Checkbutton(
            self.window,
            text="Toggle Ticker Widgets",
            var=self.ticker_widgets_visible,
            command=self.toggle_ticker_widgets,
        ).pack()

        # Toggle button for the axis set widgets
        tk.Checkbutton(
            self.window,
            text="Toggle Axis Set Widgets",
            var=self.axis_set_widgets_visible,
            command=self.toggle_axis_set_widgets,
        ).pack()
