...
class Specimen:
    ...
    def process_data(self, condition=None):
        ...
        self.data_manager.add_stress_and_strain(
            condition, recalculate=True
        )  # TODO  Marker to test if there a need to recalculate
    ...
    def from_dict(cls, data, temp_dir=None):
        ...
        specimen = cls(data["name"], None, data["length"], data["width"], data["thickness"], data["weight"])
...
class SpecimenGraphManager:
    ...
    def find_first_significant_increase(
        self, stress, strain, threshold=0.015, testing=False, min_force=80, max_force=1000
    ):
        ...
        roc_normalized = np.divide(
            rate_of_change, max_abs_rate_of_change, out=np.zeros_like(rate_of_change), where=max_abs_rate_of_change != 0
        )
        ...
        i = np.argmax(
            (roc_normalized >= threshold)
            & (self.specimen.force[:-1] >= min_force)
            & (self.specimen.force[:-1] <= max_force)
        )
        ...
        if (strain[0] - strain[i]) < (-0.04):
            ...
            i = np.argmax((strain > 0) & (self.specimen.force >= min_force) & (self.specimen.force <= max_force))
    ...
    def find_next_significant_decrease(self, stress, strain, start_index=0, threshold=0.0005, testing=False):
        ...
        if max_abs_rate_of_change == 0:
            ...
            roc_normalized = np.divide(
                ...
                out=np.zeros_like(rate_of_change),
                where=max_abs_rate_of_change != 0,
            )
    ...
    def calculate_youngs_modulus(self, stress, strain):
        ...
        if start is not None and end is not None:
            if end - start < 5:
                ...
                print(
                    "It is advised to mannually select the region by selecting 'Enable Selection Mode' in the menu bar. "
                )
    ...
    def calculate_strength(self, stress, strain, offset=0.002):
        ...
        if start is not None and end is not None and self.youngs_modulus is not None:
            offset_intercept = -(offset * self.youngs_modulus) - (
                self.youngs_modulus * strain[start]
            )  # y-intercept for the offset line
    ...
    def Calculate_Strength_Alignment(self, OFFSET=0.002):
        ...
        if self.specimen.processed_hysteresis_data is None or self.specimen.processed_hysteresis_data.empty:
            ...
            self.strain_shifted = (
                self.strain_shifted
                if not self.specimen.data_manager.has_preload_stess_and_strain
                else self.specimen.data_manager.formatted_data["strain"]
            )
    ...
    def plot_hysteresis_data(self, ax):
        ax.plot(
            self.specimen.processed_hysteresis_data["shiftd strain"],
            self.specimen.processed_hysteresis_data["stress"],
            alpha=0.5,
            color="navy",
            linestyle="--",
            label="Hysteresis Stress-Strain Curve",
        )
        ...
        if self.compressive_proof_strength is not None:
            ...
            if ps_strain is not None and ps_stress is not None:
                ax.scatter(
                    ...
                    ps_stress,
                    c="green",
                    label=f"Compressive Proof Strength: ({ps_strain:.3f}, {ps_stress:.3f})",
                )
    ...
    def from_dict(cls, data, specimen, temp_dir=None):
        ...
        for attr, value in data.items():
            if isinstance(value, str) and value.endswith("_data.csv"):
                csv_file = value
                file_path = os.path.join(temp_dir, csv_file) if temp_dir else csv_file
...
class SpecimenDataManager:
    def __init__(self, specimen, raw_data, area, original_length):
        ...
        self.COLUMN_MAPPING = {
            ...
            "force": ["force", "load"],
            "displacement": ["displacement", "elongation", "Compressive extension"],
        }
    ...
    def clean_raw_data(self):
        ...
        self.split_raw_data_df.columns = [
            "Column 1",
            "Column 2",
            "Column 3",
            "Column 4",
        ...
        if isinstance(self.hysteresis_data, pd.DataFrame):
            ...
            if recalculate:
                if "stress" in self.formatted_hysteresis_data.columns:
                    self.formatted_hysteresis_data["stress"] = (
                        self.formatted_hysteresis_data["force"].astype(float) / self.cross_sectional_area
                    )
                ...
                if "strain" in self.formatted_hysteresis_data.columns:
                    self.formatted_hysteresis_data["strain"] = (
                        self.formatted_hysteresis_data["displacement"].astype(float) / self.original_length
                    )
    ...
    def add_stress_and_strain(self, condition=None, recalculate=False):
        ...
        # check the avg of the data to see if the data needs to be inverted
        if self.formatted_data["stress"].mean() < 0:
            self.formatted_data["stress"] = self.formatted_data["stress"] * -1

        if self.formatted_data["strain"].mean() < 0:
            self.formatted_data["strain"] = self.formatted_data["strain"] * -1
    ...
    def get_modulus_from_hysteresis(self):
        ...
        # TODO. Prevously the max force and max stress were compared to ensure they occur at the same index.
        # However, it not working for current data. Need to investigate further
