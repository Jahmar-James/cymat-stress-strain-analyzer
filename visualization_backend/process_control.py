...
from visualization.control_chart import ControlChart, ControlChartPlotter, ControlProcessMetrics
...
class ControlPropertySettings:
    ...
    )  # repr=False ensures it won't show up in print statements, can be removed if not needed.
    ...
    def plot_chart(self, save_plots=False, use_sample_names=False):
        for control_property in self.control_properties:
            if control_property.df_key in self.data.columns:
                ...
                data_processor = ControlProcessMetrics(
                    property_data["column_data"], LSL=property_data["LSL"], USL=property_data["USL"]
                )
